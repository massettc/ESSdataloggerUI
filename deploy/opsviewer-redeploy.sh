...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "Create it with: sudo mkdir -p /opt/opsviewer && sudo tee $CONFIG_FILE <<'EOF'" >&2
    echo '{"EDGE_DEVICE_ID":"ESS-UNIT-XX","EventHub__ConnectionString":"Endpoint=sb://..."}' >&2
    echo "EOF" >&2
    exit 1
fi
...
echo "Starting opsviewer2-edge with EDGE_DEVICE_ID=$EDGE_DEVICE_ID ..."
docker run -d \
